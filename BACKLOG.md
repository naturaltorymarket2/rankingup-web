...
## 🔴 긴급 — 광고 승인 기능 동작 검증 (Phase 22, 2026-08-25)

> 코드 변경 완료. **migration 0038~0042 Supabase 수동 적용 필요** (미적용 시 광고 등록/미션 전체 실패).
> 적용 순서: 0038 → 0039 → 0040 → 0041 → 0042

- [ ] migration 0038~0042 Supabase SQL Editor 적용
- [ ] 적용 후 기존 운영 캠페인이 `approval_status='APPROVED'`로 소급됐는지 SQL 확인
      `SELECT approval_status, COUNT(*) FROM campaigns GROUP BY 1;`
- [ ] [광고주 웹] 광고 등록 → 포인트가 차감되지 않고 대시보드에 '승인 대기'로 표시되는지
- [ ] [어드민] `/admin/campaign` 진입 → 대기 목록에 상품명/업체명/상품URL 노출 확인
- [ ] [어드민] [상품 페이지 열기] → 태그 복사 → 붙여넣기 → 순서대로 파싱되는지 (#AAA#BBB)
- [ ] [어드민] 승인 → 포인트 차감 + campaign_tags 교체 + status ACTIVE 확인
- [ ] [어드민] 잔액 부족 광고주 승인 시 INSUFFICIENT_BALANCE 안내가 뜨고 차감/승인이 없는지
- [ ] [어드민] 거절 → REJECTED 전환, 포인트 변동 없음 확인
- [ ] [앱] 승인 전 광고가 미션 보드에 노출되지 않는지
- [ ] [앱] 승인 후 미션 시작 → 매번 다른 "N번째 태그"가 출제되는지 (랜덤 출제)
- [ ] [앱] 태그를 '#' 포함/공백 포함으로 입력해도 정답 처리되는지 (normalize_tag)
