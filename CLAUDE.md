...
### 순위와 리워드 유입의 관계 — 2026-08-31 실측

운영자 상품 3건으로 확인된 사실(기록용):

- 리워드 유입을 넣으면 **순위가 오른다** (3일 지연 반영)
- 끊으면 **원래보다 더 떨어진다**, 다시 넣으면 회복된다 (양파즙 기준 6~12위)
- 리워드를 돌리지 않은 상품은 같은 기간 순위 변화가 없었다 (대조군)
- 리워드 유입 전환율은 0.4%대 — 검색 유입 평균(1~3%)의 1/5 수준

### 유입 분산(페이싱) — 도입 후 철회 (0048 → 0049)

운영자 상품 3건으로 확인된 사실:

- 리워드 유입을 넣으면 **순위가 오른다** (3일 지연 반영)
- 끊으면 **원래보다 더 떨어진다** (전환율 희석의 영향이 뒤늦게 나타남)
- 다시 넣으면 회복된다 (양파즙 기준 6~12위 복귀)
- 리워드를 돌리지 않은 상품은 같은 기간 순위 변화가 없었다 (대조군)
- 리워드 유입 전환율은 0.4%대 — 검색 유입 평균(1~3%)의 1/5 수준

따라서 **몰아넣고 끊는 것이 가장 나쁜 운영**이다. 광고주에게는 유입 수를
그대로 팔되(순위·기간을 보장하지 않는다 — 표시광고법 위험), 언제 몇 명을
넣을지는 내부에서 곡선으로 설계한다.

| 단계 | 처리 |
|------|------|
| 승인 시 | `campaign_pacing` 에 일자별 목표 생성 (3주, 주차별 70/20/10%) |
| 하루 안 | 운영 시간 09~22시에 걸쳐 균등 배분 — 한도 초과 시 노출 중단 후 시간 경과에 따라 재개 |
| 서버 | `start_mission` 이 같은 기준으로 차단 (클라이언트 우회 방지) |
| 앱 | `get_open_mission_groups` 로 열린 그룹만 노출 + 유저별 순서 랜덤화 |

**2026-09-01 철회(migration 0049).** 광고주가 구매한 일일 유입 수는 당일에
그대로 소진될 수 있어야 한다는 운영 방침에 따라 원복했다. `campaign_pacing`
테이블과 관련 함수·트리거를 제거하고 `start_mission` 은 다시 캠페인
일일 목표(`daily_target`) 기준으로 검사한다.

유지한 것: **미션 보드 노출 순서 랜덤화**(유저·날짜 씨앗). 상단 광고 쏠림만
막고 유입량에는 영향이 없다.

0048 이 늘려 둔 `end_date` / `expires_at` 는 되돌리지 않았다. 원본 값을
남겨두지 않았고, 기간이 남아 있어도 `remaining_slots` 가 소진되면 광고는
종료되므로 판매한 유입 수에는 영향이 없다.

참여 제한은 **상품(그룹)당 하루 1회**이며 하루에 참여할 수 있는 광고 수에는
제한이 없다(기존부터 그렇게 동작해 왔다).

### 엑셀 대량 등록 (2026-09-22)

경쟁사 대비 누락돼 있던 기능. 광고주가 상품을 여러 개 운영하면 한 건씩
등록하기 번거롭다. `/web/campaign/bulk` 에서 템플릿을 내려받아 작성하고
한 번에 올린다.

| 파일 | 역할 |
|------|------|
| `campaign/domain/bulk_campaign_model.dart` | 템플릿 열 정의, xlsx 생성, 파싱·검증 |
| `campaign/presentation/campaign_bulk_screen.dart` | 3단계 화면(내려받기 → 업로드 → 확인 후 등록) |
| `shared/utils/file_download.dart` | 브라우저 내려받기 (조건부 import — 웹 전용) |

- 패키지 추가: `excel`, `file_picker`, `web`
- 업로드 즉시 등록하지 않는다. 행별 통과/오류와 차감 예정 포인트를 먼저 보여준다
- 검증: URL 형식, 필수값, 100명 단위, 시작일 내일 이후, 최소 7일, 파일 내 중복
- **대량 등록에서는 키워드 추천(SerpApi)을 하지 않는다.** 1건당 5회를 쓰기 때문에
  50건이면 250회로 월 한도(1,000회)가 금방 소진된다. 엑셀의 메인 키워드를 그대로 쓴다
- 등록 결과는 개별 등록과 동일하게 `PENDING` — 운영자 승인 시 차감·노출

> 승인은 여전히 건별 수작업(상품 페이지에서 #태그 복사)이다. 대량 등록이
> 늘면 여기가 병목이 된다. 크롤러로 태그를 미리 수집해 승인 화면에 채워
> 두는 방안이 다음 과제다(로그인된 크롬으로 상품 페이지 접근은 이미 가능 —
> `fetch_og_image` 참고).

### 상품 태그 자동 수집 (2026-09-22, migration 0050)

승인은 운영자가 상품 페이지에 들어가 #태그를 직접 복사해 붙여넣는 방식이라
건당 1~2분이 걸렸다. 엑셀 대량 등록으로 한 번에 수십 건이 들어오면 여기가
병목이 된다.

| 단계 | 처리 |
|------|------|
| 크롤러 | 일일 실행 끝에 승인 대기(PENDING) 광고의 상품 페이지를 열어 #태그 수집 |
| 저장 | `campaigns.scraped_tags` / `scraped_at` (service_role 전용 `save_scraped_tags`) |
| 어드민 | 승인 화면 입력칸에 자동으로 채워짐 + 안내 배너 표시 |
| 확정 | 지금과 동일하게 **운영자가 확인 후 승인** — 정답 풀은 승인 시점에 만들어진다 |

- 수집 태그는 **초안**이다. 잘못 긁힌 태그를 그대로 쓰면 유저가 맞힐 수 없는
  문제가 출제되므로 사람의 확인 단계를 없애지 않는다
- 입력칸은 **처음 만들 때만** 채운다. 운영자가 고친 뒤 목록이 갱신돼도 덮어쓰지 않는다
- 이미 수집된 그룹은 다시 긁지 않는다 (`scraped_tags` 가 있으면 건너뜀)
- 일반 HTTP 요청은 네이버가 429로 막는다. 로그인된 크롬(Playwright CDP)이라 통과한다
