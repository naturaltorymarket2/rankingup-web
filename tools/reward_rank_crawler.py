...
def save_thumbnail(env: Dict[str, str], product_url: str,
                   image_url: str) -> None:
    """
    상품 썸네일 URL 저장.
    ...
    썸네일은 키워드가 아니라 상품의 속성이므로, 같은 상품 URL 을 쓰는
    모든 캠페인에 한 번에 반영한다. (미션 키워드는 7일 주기로만 크롤링해서
    캠페인별로 저장하면 상당수가 썸네일 없이 남는다)
    """
    ...
    try:
        requests.patch(
            env['url'] + '/rest/v1/campaigns',
            headers=sb_headers(env),
            params={'product_url': 'eq.' + product_url},
